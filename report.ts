 ...
 * This is the lightweight version — only reads status fields, not full content.
 */
export function parseTaskStatusesFromMarkdown(reportPath: string): Map<number, SprintTask["status"]> {
	...
	// Match task sections: ### Task N: Title
	// Then look for the status row in the metadata table
...
export function syncRecordFromMarkdown(record: SprintRecord): void {
	...
	// Determine the first non-done task index for resumption
