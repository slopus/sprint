...
const FRONTEND_PATH_PATTERNS = [
	/\.tsx/i, /\.css/i, /\.html/i, /mainview/i,
	/components?\//i, /pages?\//i, /views?\//i, /hooks?\//i, /src\/app/i,
];

const FRONTEND_KEYWORDS = [
	/\bUI\b/, /\bfrontend\b/i, /\bcomponent\b/i, /\breact\b/i,
	/\bheader\b/i, /\bsidebar\b/i, /\bbutton\b/i, /\bhook\b/i,
	/\bCSS\b/, /\bstyle/i, /\blayout\b/i, /\brender/i,
	/\bpage\b/i, /\bwidget\b/i, /\bmodal\b/i, /\buse[A-Z]/,
];

export function classifyTask(title: string, body: string): boolean {
	const text = `${title}\n${body}`;
	for (const p of FRONTEND_PATH_PATTERNS) if (p.test(text)) return true;
	for (const k of FRONTEND_KEYWORDS) if (k.test(text)) return true;
	return false;
}

/**
 * Extract tasks from sprint plan markdown.
 * Tries "Task N:" format first, falls back to bold numbered items.
 */
export function parseTasks(synthesis: string): SprintTask[] {
	...
	if (matches.length === 0) {
		const fallback = /(?:^|\n)(?:#{1,4}\s*)?\*{1,2}\d+[.:]\s*(.+?)\*{1,2}\s*\n/gi;
		matches = [...synthesis.matchAll(fallback)];
	}
